...
parser.add_argument( "--final_results_dir",     type=str, default = "/home/tnguye46/T4_Lysozyme/Relative_Binding_FE/Absolute_FE/OBC2")
parser.add_argument( "--weight_scheme",         type=str, default = "__equal_sys__state_weight")
...
if len(yank_systems) > 1:
    if args.yank_systems != "all":
        ...
        final_result_file_prefix = "all_snap"
elif len(yank_systems) == 1:
    final_result_file_prefix = yank_systems[0]

else:
    raise Exception("something is wrong")

final_results_file = os.path.join(args.final_results_dir, final_result_file_prefix + args.weight_scheme, "ExpMean", args.FF + ".score")
print "Loading " + final_results_file
final_results, final_std = load_scores(final_results_file, 0, 1, 2, args.exclude_ligands) 
ligands = final_results.keys()
for ligand in final_results:
    final_results[ligand] += SITE_CORRECTION
...
for sample_size in sample_sizes:
    ...
    for absolute_fe in absolute_fes:
        r, rmse = pearson_r_and_rmse(final_results, absolute_fe)
        rs.append(r)
        rmses.append(rmse)
