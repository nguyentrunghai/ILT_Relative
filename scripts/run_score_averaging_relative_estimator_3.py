...
if os.path.basename(args.scores_dir) == "OBC2":
    print "OBC2 weights"
    from load_mbar_weights_marginal_holo_OBC2 import load_mbar_weights
...
weights = load_mbar_weights()

yank_systems = [key for key in weights.keys() if key not in ["systems"] ]
...
#TODO
FFs = MultiStruScores(args.scores_dir, ligand_groups[0], ligand_3l_codes[ligand_groups[0]][0], weights, yank_systems).get_FFs()
print FFs

for y_sys in yank_systems:
    averaging( [y_sys], y_sys + "__equal_sys__single_weight", weights )
