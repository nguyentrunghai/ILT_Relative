...
WEIGHTS_DIR = "/home/tnguye46/T4_Lysozyme/Mbar_Weights_OBC2"
...
# when unpickled we get a dict, assign[snapshot] -> list of int
# it maps each of the 576 Algdock snapshots to a list of YANK snapshots it represent
...
def load_mbar_weights():
    ...
    if len(snapshots) != (nr_selected_snapshots_per_system * nr_systems):
        raise RuntimeError("nr of snapshot from the list file is %d but number estimated from stride is %d "%(len(snapshots),
                                                                    nr_selected_snapshots_per_system * nr_systems))
    ...
    index_range_in_each_state = [(i*NR_YANK_SNAPSHOTS_PER_STATE, (i+1)*NR_YANK_SNAPSHOTS_PER_STATE)
                                 for i in range(nr_of_states_per_system)]
    ...
    for system in YANK_SYSTEMS:
        ...
        bl_weights[system] = np.array([weights[i*DOCK_STRIDE : (i+1)*DOCK_STRIDE].sum()
                                       for i in range(len(selected_snapshots_per_system))])
    ...
    all_weights_ub_weighted = np.concatenate([all_weights[system]*system_wide_weights_normalized[system]
                                              for system in YANK_SYSTEMS])
    ...
    #
    return (block_weights, state_weights, single_snap_weights,
            stru_group_weights_equal_sys, stru_group_weights_ub_weighted)
